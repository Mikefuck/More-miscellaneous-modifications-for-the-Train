...
- 页面改为**单页仪表盘**，不再分页：表头常驻抽数 / 金币 / 角色卡 / 任务进度四枚胶囊与 UTC 日切倒计时。
- 任务列表占主体版面，每行是「状态标识 → 任务名 → 说明 → 进度条与计数 → 奖励 → 领取按钮」。
- 角色卡明细与发放来源收进面板内模态弹层（点胶囊，或按 `T` / `C`）；弹层 Esc 或 ✕ 关闭。
- 面板高度按任务条数自适应，超高整行滚动；领取后按钮进入等待态直到服务端快照回来，6 秒超时自解锁。
- 文案迁移到语言键 `screen.habitrain_lottery.daily.*`（`zh_cn` / `en_us`）。每日任务 API 未变更，接入方式见 [`docs/daily-task-board.md`](docs/daily-task-board.md)。

## 1.1.19 每日任务终端

- 「每日任务终端」方块右键打开任务、当前资产与发放来源页面；本版本不预置任务。
- 其他模组可通过 `HabiDailyTaskApi` 注册任务、更新进度和处理领取。接入说明与发放路径审计见 [`docs/daily-task-board.md`](docs/daily-task-board.md)。
